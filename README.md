...
Для запуска в контейнере (необходим Docker) выполнить в терминале команду. При вводе команды собирается docker-образ и поднимается docker-контейнер. По умолчанию пробрасывается порт 8080. 
```bash
  make run
```
